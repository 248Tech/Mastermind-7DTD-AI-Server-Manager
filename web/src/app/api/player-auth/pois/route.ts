...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(`${control}/api/player-auth/pois`, { headers: { authorization: `Bearer ${token}` }, cache: 'no-store' }).catch(() => null);
  if (!response) return Response.json({ message: 'POI catalogue is unavailable' }, { status: 503 });
