...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(`${control}/api/player-auth/pois/preview?name=${encodeURIComponent(name)}`, { headers: { authorization: `Bearer ${token}` }, cache: 'no-store' }).catch(() => null);
  if (!response) return Response.json({ message: 'POI preview is unavailable' }, { status: 503 });
