...
export default function RegionHealerPage() {
  ...
  const [servers,setServers]=useState<ServerInstance[]>([]),[server,setServer]=useState<ServerInstance|null>(null),[jobs,setJobs]=useState<Job[]>([]);
  ...
  const refreshJobs=useCallback(async()=>{if(!orgId||!server)return;const recent=await api.get<Job[]>(`/api/orgs/${orgId}/jobs?limit=25`);setJobs(recent.filter(job=>job.serverInstanceId===server.id&&job.type.startsWith('REGION_HEALER_')));},[orgId,server]);
  ...
  useEffect(()=>{if(!orgId)return;api.get<ServerInstance[]>(`/api/orgs/${orgId}/server-instances`).then(rows=>setServers(rows.filter(item=>item.gameType==='7dtd'))).catch(e=>setError(e instanceof Error?e.message:'Could not load server'));},[orgId]);
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'start',gap:12,flexWrap:'wrap'}}><div><h1 style={{margin:'0 0 .25rem',color:'#f1f5f9',fontSize:'1.5rem'}}>Region Healer</h1><p style={{color:'#64748b',margin:'0 0 1.5rem'}}>Detects incorrect region headers, quarantines corrupt files, restores the latest known-good region snapshot, then restarts 7DTD.</p></div><select aria-label="Server" value={server?.id||''} onChange={event=>selectServer(event.target.value)} disabled={busy||servers.length===0} style={{background:'#111118',color:'#e2e8f0',border:'1px solid #252532',borderRadius:6,padding:'.5rem'}}><option value="">Select server</option>{servers.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
