...
function Sparkline({samples,field,color,max,windowMinutes}:{samples:HealthSample[];field:'cpuPercent'|'latencyMs'|'ramUsedMb';color:string;max:number;windowMinutes:number}) {
  if(samples.length<2)return <div style={{height:55,color:'#475569'}}>Collecting samples…</div>;
  const end=Date.now(); const start=end-windowMinutes*60_000; const span=Math.max(1,end-start);
  const points=samples.map(s=>`${Math.max(0,Math.min(100,((new Date(s.createdAt).getTime()-start)/span)*100))},${50-Math.min(50,(Number(s[field])/Math.max(max,1))*50)}`).join(' ');
...
export default function HealthPage(){
  ...
  const [error,setError]=useState(''); const [windowMinutes,setWindowMinutes]=useState(60); const [hostId,setHostId]=useState('');
  ...
  useEffect(()=>{if(!hostId&&data.hosts.length){setHostId((data.hosts.find(h=>h.status==='online')||data.hosts[0]).id);}},[data.hosts,hostId]);
  const samples=useMemo(()=>data.samples.filter(s=>!hostId||s.hostId===hostId),[data.samples,hostId]);
  const latest=samples[samples.length-1];
  const stats=useMemo(()=>({cpu:average(samples.map(s=>s.cpuPercent)),latency:average(samples.filter(s=>s.gameReachable).map(s=>s.latencyMs)),ram:average(samples.map(s=>s.ramUsedMb)),ramTotal:latest?.ramTotalMb||0}),[samples,latest]);
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',marginBottom:'1.25rem'}}><div><h1 style={{margin:0,color:'#f1f5f9',fontSize:'1.5rem'}}>Server Health</h1><p style={{color:'#64748b',margin:'.25rem 0 0'}}>Live checks and historical resource usage</p></div><div style={{display:'flex',gap:8}}>
      <select value={hostId} onChange={e=>setHostId(e.target.value)} style={{background:'#111118',color:'#e2e8f0',border:'1px solid #252532',borderRadius:6,padding:'.5rem'}}>{data.hosts.map(h=><option key={h.id} value={h.id}>{h.name} · {h.status}</option>)}</select>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1rem',marginBottom:'1rem'}}>
      <div style={card}><small style={{color:'#64748b'}}>HOST CPU · CURRENT / AVERAGE</small><div style={{fontSize:'1.7rem',color:'#38bdf8'}}>{(latest?.cpuPercent||0).toFixed(1)}% <small style={{fontSize:'.8rem',color:'#64748b'}}>/ {stats.cpu.toFixed(1)}%</small></div><Sparkline samples={samples} field="cpuPercent" color="#38bdf8" max={100} windowMinutes={windowMinutes}/></div>
      <div style={card}><small style={{color:'#64748b'}}>HOST RAM · CURRENT / AVERAGE</small><div style={{fontSize:'1.7rem',color:'#a78bfa'}}>{((latest?.ramUsedMb||0)/1024).toFixed(1)} GiB <small style={{fontSize:'.8rem',color:'#64748b'}}>/ avg {(stats.ram/1024).toFixed(1)} · total {(stats.ramTotal/1024).toFixed(1)}</small></div><Sparkline samples={samples} field="ramUsedMb" color="#a78bfa" max={stats.ramTotal} windowMinutes={windowMinutes}/></div>
      <div style={card}><small style={{color:'#64748b'}}>GAME PORT LATENCY · CURRENT / AVERAGE</small><div style={{fontSize:'1.7rem',color:'#4ade80'}}>{(latest?.latencyMs||0).toFixed(2)} ms <small style={{fontSize:'.8rem',color:'#64748b'}}>/ {stats.latency.toFixed(2)} ms</small></div><Sparkline samples={samples} field="latencyMs" color="#4ade80" max={Math.max(10,...samples.map(s=>s.latencyMs))} windowMinutes={windowMinutes}/></div>
    </div>
    <p style={{color:'#64748b',fontSize:'.75rem',margin:'-.4rem 0 1rem'}}>CPU and RAM are whole-host measurements for the selected machine. The line spans the complete selected time range and uses recorded timestamps.</p>
